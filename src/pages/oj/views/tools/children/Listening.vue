...
function getAudioByGrade(grade, chapterNum, midTerm) {
  ...
  for (let i = 1; i < chapterNum; i++) {
    ...
    if (i === midTerm + 1) {
      ...
      // 当前不是MidTerm whichExam就是undefined
      if (!givenMidTerm) {
        // 没放MidTerm之前 就是Exercise i
        _retval[index] = generateJSON(grade, i);
      } else {
        // 放了之后 就是Exercise i-1(index)
  ...
  data() {
    return {
      audios: [
        ...
        {
          titleName: "8A 听力书",
          Audiolist: getAudioByGrade("8A", 12, 6)
        },
        {
          titleName: "8B 听力书",
          Audiolist: getAudioByGrade("8B", 11, 6)
        },
        {
          titleName: "7A 个性化",
          Audiolist: [genPHData("7A", 2, 1), genPHData("7A", 4, 1)]
        },
        {
          titleName: "小练习",
          Audiolist: [
            genExerciseData(
              "上海市风华初级中学2022学年七年级第一学期国庆小练习听力"
            ),
            genExerciseData("U5单元练习听力"),
            genExerciseData("14-M3-U9-同步验收练习题"),
