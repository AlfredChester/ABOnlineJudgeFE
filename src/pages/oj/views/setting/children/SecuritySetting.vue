    ...
    <div class="flex-container setting-content">
      <template v-for="session in sessions">
        <Card :padding="20" class="flex-child">
          ...
          <div slot="extra">
            <Tag v-if="session.current_session" color="green">{{$t('m.CurrentLogin')}}</Tag>
            <Button v-else
                    type="warning"
                    size="small"
      ...
      <Form>
        ...
        <template v-if="!loadingQRcode">
          <FormItem style="width: 250px">
            <Input v-model="formTwoFactor.code" :placeholder="$t('m.Enter_The_Code')"/>
          </FormItem>
          <Button type="primary"
                  :loading="loadingBtn"
  ...
  export default {
    ...
    methods: {
      ...
      deleteSession (sessionKey) {
        this.$Modal.confirm({
          title: this.$i18n.t('m.Confirm_i18n'),
          content: this.$i18n.t('m.SureToRevokeSession'),
          onOk: () => {
      ...
      closeTFA () {
        this.$Modal.confirm({
          title: this.$i18n.t('m.Confirm_i18n'),
          content: this.$i18n.t('m.SureToCloseTFA'),
          onOk: () => {
            this.updateTFA(true)
          }
        })
