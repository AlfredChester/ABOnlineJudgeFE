<template>
  <div class="view">
    <Panel :title="$t('m.CONTEST_LIST_TITLE')">
      <div slot="header">
        <el-input
          v-model="keyword"
      ...
      <el-table
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            ...
            <p>{{ $t('m.Contest_Creater') }} : {{props.row.created_by.username}}</p>
          </template>
        </el-table-column>
        <el-table-column
          prop="id"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag :type="scope.row.contest_type === 'Public' ? 'success' : 'primary'">
              {{ scope.row.contest_type === 'Public' ? $t('m.Public') : $t('m.Password_Protected') }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-tag
              ...
              {{ scope.row.status === '-1' ? $t('m.Ended') : scope.row.status === '0' ? $t('m.Underway_i18n') : $t('m.Not_Started') }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
        ...
        <el-table-column
          ...
          <div slot-scope="scope">
            ...
            <icon-btn :name="$t('m.Problem_List')" icon="list-ol" @click.native="goContestProblemList(scope.row.id)"></icon-btn>
            <icon-btn :name="$t('m.Release_AnnounceMent')" icon="info-circle"
                      @click.native="goContestAnnouncement(scope.row.id)"></icon-btn>
            <icon-btn icon="download" :name="$t('m.Download_AC_Submission')"
    ...
    <el-dialog :title="$t('m.Download_Submission')"
               width="30%"
