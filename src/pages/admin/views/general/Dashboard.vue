      ...
      <el-card class="admin-info">
        <el-row :gutter="20">
          ...
          <el-col :span="14">
            ...
            <p>{{ user.admin_type === 'Regular User' ? $t('m.Regular_User') : (user.admin_type === 'Admin' ? $t('m.Admin') : $t('m.Super_Admin')) }}</p>
          </el-col>
        </el-row>
        <hr/>
        <div class="last-info">
      ...
      <panel :title="$t('m.System_Overview')" v-if="isSuperAdmin">
        ...
        <p>{{$t('m.HTTPS_Status')}}:
          <el-tag :type="https ? 'success' : 'danger'" size="small">
            {{ https ? $t('m.Enabled_i18n') : $t('m.Disabled_i18n')}}
          </el-tag>
        </p>
        <p>{{$t('m.Force_HTTPS')}}:
          <el-tag :type="forceHttps ? 'success' : 'danger'" size="small">
            {{forceHttps ? $t('m.Enabled_i18n') : $t('m.Disabled_i18n')}}
          </el-tag>
        </p>
        <p>{{$t('m.CDN_HOST')}}:
          <el-tag :type="cdn ? 'success' : 'warning'" size="small">
            {{cdn ? cdn : $t('m.Disabled_i18n')}}
          </el-tag>
        </p>
      </panel>
    </el-col>
    ...
    <el-col :md="14" :lg="16" v-if="isSuperAdmin">
      ...
      <panel style="margin-top: 5px">
        ...
        <el-popover placement="right" trigger="hover">
          ...
          <p>{{$t('m.Please_Upgrade_To_Enjoy')}}</p>
          <p>{{$t('m.Reference_Show')}} <a href="http://docs.onlinejudge.me/#/onlinejudge/guide/upgrade" target="_blank">
        ...
        <el-collapse v-model="activeNames" v-for="(release, index) of releases" :key="'release' + index">
          <el-collapse-item :name="index+1">
            ...
            <p>{{$t('m.Release_Level')}} {{release.level === 'Important' ? $t('m.Important_Release_Level') : $t('m.Recommend_Release_Level')}}</p>
            <p>{{$t('m.Release_Details')}}</p>
