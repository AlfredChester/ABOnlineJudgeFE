...
<script>
  ...
  export default {
    ...
    data () {
      return {
        rules: {
          _id: {required: true, message: this.$i18n.t('m.Display_ID_is_required'), trigger: 'blur'},
          title: {required: true, message: this.$i18n.t('m.Title_is_required'), trigger: 'blur'},
          input_description: {required: true, message: this.$i18n.t('m.Input_Description_is_required'), trigger: 'blur'},
          output_description: {required: true, message: this.$i18n.t('m.Output_Description_is_required'), trigger: 'blur'},
          description: {required: true, message: this.$i18n.t('m.Question_Description_is_required'), trigger: 'blur'}
        },
    ...
    methods: {
      switchSpj () {
        if (this.testCaseUploaded) {
          this.$confirm(this.$i18n.t('m.If_you_change_spj'), this.$i18n.t('m.Warning_i18n'), {
            confirmButtonText: this.$i18n.t('m.I_understood'),
            cancelButtonText: this.$i18n.t('m.Cancel_i18n'),
            type: 'warning'
          }).then(() => {
      ...
      submit () {
        ...
        if (this.problem.spj) {
          if (!this.problem.spj_code) {
            ...
            this.error.spj = this.$i18n.t('m.SPJ_code_Compile_Failed')
          }
          if (this.error.spj) {
            this.$error(this.error.spj)
            return
        ...
        if (!this.problem.languages.length) {
          this.error.languages = this.$i18n.t('m.Please_choose_at_least_one_language')
          this.$error(this.error.languages)
          return
        }
        if (!this.testCaseUploaded) {
          this.error.testCase = this.$i18n.t('m.Testcase_is_not_uploaded_yet')
